...
void* split(node_t* node,size_t needed_size){
  ...
  if (node->prev == NULL){              
        ...
        new_node->block_size = node->block_size - (needed_size + sizeof(size_t)) ;
  ...
  if (node->next == NULL) //found that the next equals NULL 
  {
     node_t* temp_node = (node_t*) sbrk(SBRK_INCREMENT) ;//move the SBRK 
      if ( (void *) temp_node == (void*) -1 ){
        ...
        if ((node_t*)((unsigned char*)node+(node->block_size + sizeof(size_t))) == temp_node){
          //merge them 
          node->block_size +=temp_node->block_size + sizeof(size_t) ;
...
void merge(node_t* left_node, node_t* right_node) {
  ...
  if ((node_t*) ((unsigned char*)left_node+(left_node->block_size)+sizeof(size_t)) == (node_t*)  right_node ){
...
void check_on_tail_size(node_t* tail){
 ...
 void* shifted_tail = (void*) ((unsigned char*)tail+(tail->block_size)+sizeof(size_t)) ; 
 if (tail->block_size >= (SBRK_INCREMENT- sizeof(size_t)))
{ 
...
void HMM_free(void* ptr){
  ...
  void* HMM_realloc(void* old_ptr, size_t new_size){
    void* new_ptr = NULL ;
    size_t old_size = *(size_t*)((unsigned char*)old_ptr-sizeof(size_t)) ;
...
/*
 int main ()
 {
  void* current_pb = sbrk(0) ; // 0x555555559000
   void* ptr5 = HMM_malloc(4*4096) ;
    void* ptr6 = HMM_malloc(4*4096) ;
    current_pb = sbrk(0) ; // 0x555555562000
        HMM_free(ptr6) ;
        current_pb = sbrk(0) ;//;0x55555555d008
        HMM_free(ptr5) ;
  current_pb = sbrk(0) ;
 while (1) ;
